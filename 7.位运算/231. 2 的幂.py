...
class Solution:
    ...
    假设 n 的二进制表示为 (a10⋯0) ，其中a 表示若干个高位，1 表示最低位的那个 1，
    0⋯0 表示后面的若干个 0，那么 n-1的二进制表示为：(a01⋯1)
    我们将(a10⋯0)与(a01⋯1) 进行按位与运算，高位 a 不变，在这之后的所有位都会变为 0，
    这样我们就将最低位的那个 1 移除了。
    ...
    那么 −n 的二进制表示为：
    (a'01……1)+(1)=(a'10……0)
    其中 a'表示将 a 每一位取反。我们将 (a10⋯0)与 (a'01……1)进行按位与运算，高位全部变为 0，
    最低位的 1 以及之后的所有 0 不变，这样我们就获取了 n 二进制表示的最低位的 1。
